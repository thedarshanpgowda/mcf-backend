...
async function processData(req, res) {
  try {
    let { startDateTime, endDateTime, thresholds, gsat_number } = req.body;
    console.log(startDateTime, endDateTime, thresholds, gsat_number);
    const fileStream = fs.createReadStream('input2.out');
    ...
    thresholds = thresholds ? thresholds : {
      "yaw": { "min": -1.49, "max": -1.47 },
      "roll": { "min": -3.32, "max": -3.33 },
      "pitch": { "min": 12.6, "max": 12.8 }
    }


    gsat_number = gsat_number ? gsat_number : "GSAT-9";

    let [sdate, stime] = startDateTime ? startDateTime.split('T') : ["2024-08-21", "00:00:00"];
    let [edate, etime] = endDateTime ? endDateTime.split('T') : ["2024-08-25", "23:59:59"];
    ...
    rl.on('close', async () => {
      ...
      const hrlyData = await ResultModel.create({
        ...
        result: hourlyData.slice(0, 30),
        thresholdArr: thresholdArr.slice(3, 30),
        inactivity: stablePeriod.slice(0, 30),
        stabilityArr: thresholdViolationPeriod.slice(0, 30)
      });
